...
# usage: ./id-to-hash.sh AGKcecrgyBNHkgjKqDlQPW7YlimD+VXPfpAx+gLSR0n1cOdIxFHaxKEF8SgAoaEtqQ==
...
then
  ...
  hashcat="sha256:$(printf '%d' 0x$iter):$salt:$subkey"
  salt=${salt//+/\.}
  salt=${salt//=/}
  subkey=${subkey//+/\.}
  subkey=${subkey//=/}
  john="\$pbkdf-sha256\$$(printf '%d' 0x$iter)\$$salt\$$subkey"
elif [ 00 == $kind ]
then
  ...
  iter=3e8
  hashcat="sha1:$(printf '%d' 0x$iter):$salt:$subkey"
  salt=${salt//+/\.}
  salt=${salt//=/}
  subkey=${subkey//+/\.}
  subkey=${subkey//=/}
  john="\$pbkdf-sha1\$$(printf '%d' 0x$iter)\$$salt\$$subkey"
else
